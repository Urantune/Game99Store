...
import webbackend.service.ImageGameService;
...
public class GameCore {
    ...
    void initBasePath() {
        ...
        try {
            ...
            throw new RuntimeException("Base path error");
        }
    }

    public ImageGame imageLinkGame(Game game) {
        return imageGameService.findByGameId(game.getGameId());
    }

    public static String[] deceptionGame(String s) {
    ...
    public void payMent(Users user, Game game, Vouncher v) {
    }
    ...
    public static class DownloadGame {
        ...
        GameRepository gameRepository;
        @GetMapping("/download")
        public ResponseEntity<Resource> download(@RequestParam("path") String path, @RequestParam("gameId") UUID gameId, HttpSession session) {
            try {
                ...
                if (ug == null || ug.getStatus().equalsIgnoreCase("owned") == false)
